...
class Watcher(threshold: ActorRef, keepers: ActorRef, lurker: ActorRef, gatewayServer: ActorRef) extends Actor {
  ...
  def receive: Receive = {
    case 'Wake => {
      manipulateGateways(gatewayActions)
    }
    case 'Close => {
      scheduleCloseOnTransients()
      // Flush messages are generated by the above
    }
    ...
    case 'Disempower => {
      Logger.debug("Watcher become unempowered")
      context.become(unempowered)
    }
    case 'Ping => {
      sender ! 'Pong
    }
  ...
  def unempowered: Receive = {
    case 'Wake => {
      manipulateGateways(unempoweredGatewayActions)
    }
    case 'Close => {
      scheduleCloseOnTransients()
      // Flush messages are generated by the above
    ...
    case OpenGateSuccess(g, lp) => {
      // Ignore the open, just mark it transient
      markTransient(g)
    }
    case OpenGateFailed(g) => {
      markTransient(g)
  ...
  private def gatewaysWantedOpen: Set[Gateway] = {
  ...
  private def gatewayActions: (Set[Gateway], Set[Gateway], Set[Gateway]) = {
    val wantOpen = gatewaysWantedOpen
    ...
    (toOpen, toTransient, keepOpen)
  }
  
  private def unempoweredGatewayActions: (Set[Gateway], Set[Gateway], Set[Gateway]) = {
    val toTransient = openQuery.toList.toSet
    Logger.debug("Watcher CHANGE: " + (Map.empty ++ toTransient.map("transient" -> _)))
    (Set.empty, toTransient, Set.empty)
  }
  
  private def manipulateGateways(actions: => (Set[Gateway], Set[Gateway], Set[Gateway])) {
    transaction ({
      val (toOpen, toTransient, keepOpen) = actions
  ...
  private def scheduleCloseOnTransients() {
    val watcher = self
    transaction {
  ...
  private def closeGateway(gatewayId: Long) {
    transaction {
