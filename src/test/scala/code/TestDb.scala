...
object TestDb extends Db {
  ...
  def reset {
    ...
    transaction {
      ...
      val cow = gateways.insert(new Gateway(bob.id, "10.16.15.43/public", "frog/sheep/cow", "", "cowsaregreen", GateMode.rw, GateState.lost))
      val goat = gateways.insert(new Gateway(jane.id, "10.16.16.16/share", "goat.tc", "", "nattyNAT", GateMode.rw, GateState.lost))
